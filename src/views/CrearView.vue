<template>
    <div class="container">
        <h3 class="text-muted mb-3">Crear factura</h3>
        <form @submit.prevent="guardarFactura()" class="mb-4">
            ...
            <button type="submit" class="btn btn-primary" v-if="form.detalles.length">Guardar Factura</button>
    ...
    methods: {
        ...
        guardarFactura: async function(){
            try {
                ...
                const response = await axios.post('http://127.0.0.1:8001/api/factura', factura , CONFIG);
                this.$toastr.s('La factura se creo correctamente.');
                this.$router.push("/");
            } catch (error) {
                this.$toastr.e('Error al crear la factura.');
            }
        },
